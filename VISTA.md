...
| 2026-08-02 (final) | One card per accident — WITHOUT ever suppressing a dispatch: the earlier `fusion_grace_s=15` pipeline merge was REVERTED (it could swallow a REAL second crash that happened at the same spot within 15s as a "late sub-event" — this caused one accident in the demo clip to be completely missed). Merging now lives in the GUI and is DISPLAY-ONLY: `gui_app.py` got `INCIDENT_MERGE_S=6.0` + `INCIDENT_MERGE_PX=200.0` and `VideoWorker._nearest_incident()` — an alert joins an open incident if it's within the time window AND (shares a track OR is within 200px OR is the closest open incident). Sub-alerts of an already-displayed crash don't create a new card, don't duplicate screenshots, and their video clip is folded into the primary card's Play button (`_find_clip_owner`). Every dispatch is still written to `alerts.jsonl` — the merge only decides how many cards the operator sees. Pipeline fusion stays at fast 6s/120px track-or-spot only. `test_scenario.py` + `test_accuracy.py` (incl. collision→smoke `dispatched==1`) all PASS; verified with a headless incident-merge simulation. |
| 2026-08-02 (revert) | Reverted today's accident-detection tuning back to yesterday's (0a33638) accuracy profile — A/B runs showed today's tuning gained nothing: clip_03 (head-on) LOST its collision alert entirely (0 vs 2) and clips were split into `jerk`/`smoke` false alerts instead of the real `collision`. Root cause: `active_ids()` in `track_history.py` was restricted to tracks detected on the MOST RECENT frame, so on the overlap frame one vehicle briefly dropping a detection starved the collision IoU pair → never checked → 0 alerts. Restored the full buffered-track set. Also reverted: `speed_drop` to windowed `velocity_between` (ratio 0.8 / min prior 2.0, Kalman `now_v` removed), removed `check_jerk` (wall crashes already covered by `speed_drop`/`collision`), `smoke_detector_enabled` now defaults `False` (its extra `smoke` card was masking the real collision), and `instantaneous_velocity` reads an LS fit again (no Kalman override) so the pre/post-impact speed collapse isn't smoothed away. Kept today's good parts: jam suppression (pixel fallback + prior-motion), severity smoke/jerk scorers (dormant), fusion + one-card GUI merge, clip/plate/multi-camera. Verified: clip_01 → 1 alert (`speed_drop` medium), clip_03 → 2 alerts (`collision` medium + `speed_drop` medium) — matches yesterday's numbers exactly; `test_scenario.py` + `test_accuracy.py` all PASS. |
| 2026-08-02 (merge undo) | Undid the "one card per crash" display merge — its time-window fallback swallowed real accidents. `gui_app.py` `_nearest_incident()` (which merged ANY alert within `INCIDENT_MERGE_S=6.0` of an open incident even when it was a DIFFERENT crash elsewhere in the scene, via the `if best is None: best = inc` fallback) now returns None: every dispatch creates its OWN card + screenshot set again. Lowered pipeline `fusion_window_s` from 6.0 back to the 1.5s baseline (a 6s window fused the 2nd/4th accidents into the 1st/3rd — same spot, seconds apart). Verified on `Downloads/demo1.mp4`: all four crash clusters now dispatched separately (t=2.0/2.4/5.2, 25.5/27.0, 58.8/62.3, 73.6/73.7). `test_scenario.py` + `test_accuracy.py` all PASS. |
| 2026-08-02 (remove smoke/jerk) | Fully removed the `smoke` and `jerk` detection features (their extra alert cards split real crashes into noise and masked the real `collision`): deleted `smoke_detector.py`, removed jerk/smoke severity scorers + baselines from `severity.py`, removed jerk/smoke from `fusion.py` `EVENT_PRIORITY` + smoke-evidence carry, removed jerk/smoke config thresholds + verification windows, removed `deceleration()` from `track_history.py`/`speed_estimator.py` + the GUI one-card merge comment. Real clips revalidated after removal - clip_03: `collision`+`speed_drop` (2 alerts), clip_01: `speed_drop` (1 alert) - unchanged. `test_accuracy.py` rewritten (Jerk/smoke tests dropped, `test_collision()` asserts a head-on collision dispatches) + `test_scenario.py`: all PASS. |
