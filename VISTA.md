...
| 2026-08-02 (latest) | Fixed "same accident detected again as another criterion": a single crash first fire jerk/collision, then seconds later re-confirmed as `smoke`/`anomaly_stop` → dock = multiple cards. Verifier now still CONFIRMS every kind (evidence kept for severity), but the fuser gained a GRACE window (`fusion_grace_s=15` in `HeuristicConfig`): a delayed sub-event arriving within the grace window that is spatially close to an existing incident JOINS it (won't the smoke's `has_smoke` severity boost). Only within track-matches stay on the fast window. Test: collision + delayed smoke at same spot → dispatched == 1 with `has_smoke` carried onto the incident. |
